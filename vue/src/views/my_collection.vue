  ...
  <div>
    <div class="container">
      <el-table :data="filteredData" border class="table" ref="multipleTable" header-cell-class-name="table-header"
        ...
        <el-table-column prop="name" label="车辆型号" align="center"></el-table-column>
        <el-table-column prop="color" label="颜色" align="center"></el-table-column>
        <el-table-column prop="daily_rent" label="租金" align="center">
          <template #default="scope">
            <span>￥ {{ scope.row.daily_rent }} /天</span>
          </template>
        </el-table-column>
        <el-table-column prop="seat_number" label="座位数" width="100" align="center"></el-table-column>
        <el-table-column prop="total_rent_number" label="历史租赁次数" align="center"></el-table-column>
        <el-table-column prop="description" label="简介" align="center">
          <template #default="scope">
            <span>{{ truncateText(scope.row.description) }}</span>
          </template>
        </el-table-column>
...
const handleCancell = async (id: number) => {
  ElMessageBox.confirm('确定取消收藏该车辆吗？', '提示', {
    type: 'warning'
  }).then(async () => {
    try {
