  ...
  <div>
    ...
    <div class="container">
      <div class="search-box">
        <el-input v-model="searchKey" placeholder="请输入搜索内容" class="search-input mr10" clearable></el-input>
      ...
      <el-table :data="filteredData" border class="table" ref="multipleTable" header-cell-class-name="table-header"
        ...
        <el-table-column prop="role_id" label="角色" align="center">
          <template #default="scope">{{ findRoleName(scope.row.role_id) }}</template>
        </el-table-column>
        <el-table-column prop="register_time" label="注册时间" align="center">
          <template #default="scope">{{ formatDateTime(scope.row.register_time) }}</template>
        </el-table-column>
...
import TableEdit from './user-edit.vue';
import {useRoute, useRouter} from "vue-router";
import moment from 'moment';
import {formatDateTime} from '../utils/timehandle';

//这个接口里的字段名必须要和后端返回的接口字段一致，否则无法显示
interface TableItem {
  id: number;
  name: string;
  email: string;
...
const filteredData = computed(() => {
  ...
  return tableData.value.filter(item => {
    return (
        ...
        item.email.toString().includes(keyword) ||
        item.description.toString().includes(keyword)
    );
  });
});
