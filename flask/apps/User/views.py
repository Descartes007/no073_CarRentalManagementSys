...
def login():
    # 向数据库查询该管理员角色信息
    if int(request.get_json()['role']) == 1:
        user = Admin.query.filter(and_(Admin.username == request.get_json()['username'],Admin.role_id == int(request.get_json()['role']))).first()
    elif int(request.get_json()['role']) ==2 :
        user = User.query.filter_by(username=request.get_json()['username']).first()
    if user is not None:
        query_password = user.query.filter_by(username=request.get_json()['username']).first().password
        if query_password == request.get_json()['password']:
            ...
            redis_store.set(user.username + "-refresh_token", refresh_token)
            user.last_login_time = datetime.now()
            user.ip = request.remote_addr
            db.session.add(user)
            db.session.commit()
            response = make_response({'status': True, 'message': '登录成功', 'token': token, 'refresh_token': refresh_token})
        ...
        response = make_response({'status': False, 'message': '登录失败，没有此用户'})
    return response
...
def logout(current_user):
    ...
    # audit_logger.debug(
    #     "[{0}] [{1}] [{2}] [退出登录]".format(request.remote_addr, request.get_json()['username'], model_name))
        ...
        if len(request.get_json()) == 0:
            ...
            if "query_string" in request.get_json():
                ...
                # 指定关键字查询
                    ...
                    query_result = User.query.filter((User.username.contains(query_string))).all()
                    pageTotal = len(query_result)
            else:
                # 指定用户名查询
                query_result = Admin.query.filter_by(username=request.get_json()['name']).all()
    else:
        query_result=User.query.filter_by(username=current_user).all()
        pageTotal = len(query_result)
    for i in query_result:
        user_dict = i.to_dict()
        register_time = int(i.to_dict()['register_time'].timestamp())
        last_login_time = int(i.to_dict()['last_login_time'].timestamp())
...
def user_permission_query():
    ...
    for i in User.query.all():
        data = {'name': i.username, 'permissions': Role.query.filter_by(id=i.role_id).first().permission_ids}
...
def user_center_update(current_user):
    ...
    if role=="1":
        user = Admin.query.filter_by(username=request.get_json()["name"]).first()
        if user is None:
            response = {"code": -1, "message": "用户不存在!", "success": False}
        else:
            query_oldpwd = getattr(Admin.query.filter_by(username=request.get_json()["name"]).first(), 'password')
            logger.debug("数据库中查询结果:" + query_oldpwd)
            if query_oldpwd != request.get_json()['oldpwd']:
                response = {"code": -1, "message": "旧密码错误!", "success": False}
            else:
                try:
                    Admin.query.filter_by(username=request.get_json()["name"]).update({'password': request.get_json()["newpwd"], 'description': request.get_json()["description"]})
                    # 提交更新到数据库（事务提交）
                    db.session.commit()
        ...
        if user is None:
            ...
            query_oldpwd = getattr(User.query.filter_by(username=request.get_json()["name"]).first(), 'password')
            logger.debug("数据库中查询结果:" + query_oldpwd)
            if query_oldpwd != request.get_json()['oldpwd']:
                response = {"code": -1, "message": "旧密码错误!", "success": False}
            else:
                try:
                    User.query.filter_by(username=request.get_json()["name"]).update(
...
def user_update(current_user):
    ...
    if user is None:
        ...
        try:
            User.query.filter_by(username=request.get_json()["username"]).update(
            ...
            response = {"code": 0, "message": "用户信息更新成功!", "status": True}
            insert_audit_log(request.remote_addr, current_user, '用户管理',
                             '更新' + request.get_json()['username'] + '用户信息')
        except Exception as e:
            logger.exception(e)
            db.session.rollback()
    ...
    for user_id in delete_ids:
        ...
        if query_result is None:
            ...
            try:
                ...
                # audit_logger.debug(
                #     "[{0}] [{1}] [{2}] [删除{3}用户信息]".format(request.remote_addr, current_user, '用户管理',
                #                                                  query_result.name))
                insert_audit_log(request.remote_addr, current_user, '用户管理',
...
def user_register():
    # 此处向数据库查询有无冲突并添加用户返回给前端添加结果
    users = User.query.filter_by(username=request.get_json()["username"]).first()
    if users is None:
        max_id_query = db.session.query(func.max(User.id)).scalar()
        if max_id_query is None:
            max_id = 0
            ...
            max_id = max_id_query
        try:
            col = User(u_id=max_id + 1, username=request.get_json()["username"], password=request.get_json()["password"],
                       email=request.get_json()["email"], phone=request.get_json()["phone"],role_id=2)
            col.register_time = datetime.now()
            db.session.add(col)  # 添加一条
            # 提交事务
            db.session.commit()
        ...
        if request.get_json()["email"] != user.email:
            ...
            if send_mail(settings.Config.EMAIL_USERNAME, request.get_json()["email"], '用户密码找回',
                ...
                try:
                    ...
                    # audit_logger.debug(
                    #     "[{0}] [{1}] [{2}] [重置密码]".format(request.remote_addr, request.get_json()["username"],
                    #                                           '用户管理'))
